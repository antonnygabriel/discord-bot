...
module.exports = {
  ...
  async execute(client, interaction) {
    // Verifica se é o subcomando 'add'
      ...
      try {
        ...
        if (targetMember.roles.cache.has(role.id)) {
          const errorEmbed = new EmbedBuilder()
            .setColor('#FF0000')
            .setTitle('❌ Erro')
            .setDescription(`${targetUser.tag} já possui o cargo ${role.name}.`)
            .setTimestamp();
        ...
        if (interaction.member.roles.highest.position <= role.position && interaction.guild.ownerId !== interaction.user.id) {
          const errorEmbed = new EmbedBuilder()
            .setColor('#FF0000')
            .setTitle('❌ Erro')
            .setDescription(`Você não pode adicionar o cargo ${role.name} pois ele é superior ou igual ao seu cargo mais alto.`)
            .setTimestamp();
        ...
        const successEmbed = new EmbedBuilder()
          ...
          .setDescription(`O cargo ${role} foi adicionado a ${targetUser.tag}.`)
          .addFields(
            { name: 'Adicionado por', value: interaction.user.tag },
        ...
        const errorEmbed = new EmbedBuilder()
          .setColor('#FF0000')
          .setTitle('❌ Erro')
          .setDescription(`Ocorreu um erro ao tentar adicionar o cargo ${role.name} a ${targetUser.tag}.`)
